...
export default function unauthorized(this: ExpressResponse, data?: any, message?: string): void {
  ...
  if (processedData) {
    res.status(401).json({ status: 'error', data: processedData, message: message ? message : 'unauthorized' });
  } else {
    res.status(401).json({ status: 'error', message: message ? message : 'unauthorized' });
  }
}
